...
export class AfricasTalkingProvider implements ISmsProvider {
    ...
    private getConfig() {
        if (this.apiUrl === null) {
            ...
            if (this.environment === 'sandbox') {
                this.apiUrl = process.env.AFRICASTALKING_API_URL || 'https://api.sandbox.africastalking.com/version1/messaging';
            } else {
                this.apiUrl = process.env.AFRICASTALKING_API_URL || 'https://api.africastalking.com/version1/messaging';
            }
     ...
     * Reference: https://help.africastalking.com/en/articles/742491-why-did-my-messages-fail
     */
    private getErrorMessage(statusCode: number, status: string, recipient: any, defaultMessage: string): string {
    ...
    private formatPhoneForAfricasTalking(phoneNumber: string): string {
        ...
        if (cleanPhone.startsWith('07')) {
            ...
            throw new Error(`Invalid phone number format for AfricasTalking SMS: ${phoneNumber}. Expected format: 07XXXXXXXX or 2547XXXXXXXX`);
        }
    ...
    async sendSms(phoneNumber: string, message: string): Promise<SmsResult> {
        ...
        if (!this.isConfigured()) {
            return {
                success: false,
                error: 'AfricasTalking SMS credentials not configured. Please set AFRICASTALKING_USERNAME and AFRICASTALKING_API_KEY in your environment variables.',
            };
        }
            ...
            if (result.SMSMessageData) {
                ...
                if (recipients.length > 0) {
                    ...
                    if (isSuccess) {
                        return {
                            success: true,
                            messageId: recipient.messageId && recipient.messageId !== 'None' ? recipient.messageId : undefined,
                            metadata: {
                                apiResponse: result,
                                formattedPhone,
                                message: message,
