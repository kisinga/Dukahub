...
export class InAppActionHandler implements IChannelActionHandler {
  ...
  private getMessageForEvent(event: ChannelEvent): string {
    const messageMap: Record<string, (data: any) => string> = {
      ...
      ml_extraction_queued: data =>
        `Photo extraction has been queued for channel ${data.channelId || 'N/A'}`,
      ml_extraction_started: data =>
        `Photo extraction has started for channel ${data.channelId || 'N/A'}`,
