...
export async function isDatabaseEmpty(): Promise<boolean> {
    ...
    try {
        ...
        throw new Error(`Failed to check database state: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
        await pool.end();
    }
}
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            if (missingTables.length === 0) {
                ...
                if (attempt < maxRetries) {
                    console.log(`⏳ Waiting for tables: ${missingTables.join(', ')} (attempt ${attempt}/${maxRetries})`);
                    await new Promise(resolve => setTimeout(resolve, retryDelayMs));
                } else {
