    ...
    async allocatePaymentToPurchases(
        ...
        return this.connection.withTransaction(ctx, async (transactionCtx) => {
            try {
                ...
                let unpaidPurchases = await this.getUnpaidPurchasesForSupplier(transactionCtx, input.supplierId);
                ...
                const purchasesPaid: Array<{ purchaseId: string; purchaseReference: string; amountPaid: number }> = [];
                const purchaseRepo = this.connection.getRepository(ctx, StockPurchase);
                ...
                for (const allocation of calculation.allocations) {
                    ...
                    const currentPaidAmount = allocationItems.find(item => item.id === allocation.itemId)?.paidAmount || 0;
                ...
                const remainingUnpaidPurchases = await this.getUnpaidPurchasesForSupplier(transactionCtx, input.supplierId);
