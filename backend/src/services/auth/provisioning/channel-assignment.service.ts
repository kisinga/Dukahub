...
import { Channel, ID, PaymentMethod, RequestContext, StockLocation, TransactionalConnection } from '@vendure/core';
...
export class ChannelAssignmentService {
    ...
    private async verifyStockLocationAssignment(
        ...
        if (!verifiedChannel || !verifiedChannel.stockLocations?.some(sl => sl.id === stockLocationId)) {
            throw new Error(
                `REGISTRATION_STOCK_LOCATION_ASSIGN_FAILED: ` +
    ...
    private async verifyPaymentMethodAssignment(
        ...
        if (!verifiedChannel || !verifiedChannel.paymentMethods?.some(pm => pm.id === paymentMethodId)) {
            throw new Error(
                `REGISTRATION_PAYMENT_METHOD_ASSIGN_FAILED: ` +
