...
export class RoleProvisionerService {
    ...
    private async createRoleViaRepository(
        ...
        if (!roleWithChannels) {
            throw this.errorService.createError('ROLE_CREATE_FAILED', 'Failed to load role after creation');
        }
     ...
     */
    private async verifyRoleChannelLinkage(ctx: RequestContext, roleId: ID, channelId: ID): Promise<void> {
        const roleRepo = this.connection.getRepository(ctx, Role);
        ...
        if (!verifiedRole) {
            throw this.errorService.createError('ROLE_ASSIGN_FAILED', 'Failed to load role for verification');
        }
