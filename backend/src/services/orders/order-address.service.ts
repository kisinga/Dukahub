...
export class OrderAddressService {
    ...
    private async getAddressInput(
        ...
        if (customerId) {
            ...
            if (customer && customer.addresses && customer.addresses.length > 0) {
                ...
                return {
                    fullName: customerAddress.fullName || `${customer.firstName || ''} ${customer.lastName || ''}`.trim() || 'Customer',
    ...
    private async applyAddresses(
        ...
        const addressMetadata = connection.entityMetadatas.find(
            meta => meta.tableName === 'address'
        );
