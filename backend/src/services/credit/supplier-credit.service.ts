...
export class SupplierCreditService {
  ...
  async getSupplierCreditSummary(
    ctx: RequestContext,
    supplierId: ID
  ...
  async updateSupplierCreditDuration(
    ...
    await this.connection.getRepository(ctx, Customer).save(supplier);
    this.logger.log(
      `Updated supplier credit duration for supplier ${supplierId} to ${creditDuration} days`
    );
  ...
  private mapToSummary(supplier: Customer, outstandingAmount: number): SupplierCreditSummary {
    ...
    const lastRepaymentDate = customFields?.supplierLastRepaymentDate
      ? new Date(customFields.supplierLastRepaymentDate)
      : null;
