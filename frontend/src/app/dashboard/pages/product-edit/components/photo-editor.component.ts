        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3">
          @for (photo of allPhotos(); track $index; let i = $index) {
            <div class="relative aspect-square group">
              ...
              <!-- New Photo Badge -->
              @if (isNewPhoto(i)) {
                <div class="badge badge-xs badge-success absolute top-1 left-1">New</div>
              }
            </div>
          }
        </div>
      ...
      @if (hasChanges()) {
        <div class="alert alert-info">
          ...
          <div class="text-sm">
            ...
            {{ newPhotos().length }} new, {{ removedPhotos().length }} removed
          </div>
        </div>
...
export class PhotoEditorComponent implements OnInit {
  ...
  private updateAllPhotos(): void {
    ...
    const filteredExisting = existingAssets.filter((asset) => !removedPhotos.includes(asset.id));
    ...
    // Update hasChanges
    this.hasChanges.set(newPhotos.length > 0 || removedPhotos.length > 0);
  }
