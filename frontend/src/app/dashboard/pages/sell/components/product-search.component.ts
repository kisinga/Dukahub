    ...
    <div class="card bg-base-100 shadow-lg">
      <div class="card-body p-3">
        <div class="flex items-center gap-2">
          ...
          <input
            type="text"
            class="input input-ghost flex-1 text-base font-medium p-0 focus:outline-none min-h-0 h-auto"
            placeholder="Search products..."
            [(ngModel)]="searchTerm"
            (ngModelChange)="onSearchChange($event)"
          />

          <!-- Loading / Scanner Toggle -->
          @if (isSearching()) {
          <span class="loading loading-spinner loading-sm"></span>
          } @else {
          <button
            class="btn btn-circle btn-sm"
            [class.btn-primary]="isScannerActive()"
            [disabled]="!canStartScanner()"
            (click)="scannerToggle.emit()"
            [attr.aria-label]="isScannerActive() ? 'Stop' : 'Scan'"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-5 w-5"
        ...
        <div class="mt-2 space-y-1 max-h-[60vh] overflow-y-auto">
          ...
          <button
            class="w-full flex items-center gap-2 p-2 rounded-lg bg-base-200 hover:bg-base-300 active:scale-[0.98] transition-transform"
...
export class ProductSearchComponent {
  ...
  readonly isScannerActive = input<boolean>(false);
  readonly canStartScanner = input<boolean>(false);

  // Outputs
  readonly searchTermChange = output<string>();
  readonly productSelected = output<ProductSearchResult>();
  readonly scannerToggle = output<void>();
