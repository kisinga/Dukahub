                        ...
                        <div class="card-body p-4">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-base text-base-content">{{ line.productVariant.name }}</h4>
                                    @if (line.productVariant.sku) {
                                        <p class="text-xs text-base-content/60 mt-1">SKU: {{ line.productVariant.sku }}</p>
                                    }
                                </div>
                                <div class="text-right ml-3 shrink-0">
                                    <div class="font-bold text-lg text-primary">{{ formatCurrency(line.linePriceWithTax) }}</div>
                                </div>
                            </div>
                            <div class="flex justify-between items-center pt-2 border-t border-base-300/50">
                                <span class="text-sm text-base-content/70">Quantity: <strong>{{ line.quantity }}</strong></span>
                                <span class="text-sm text-base-content/70">Unit: <strong>{{ formatCurrency(line.linePriceWithTax / line.quantity) }}</strong></span>
                            </div>
                        </div>
                    </div>
                }
