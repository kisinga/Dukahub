            ...
            <div class="card-body p-4 sm:p-5">
                ...
                <div class="flex items-start justify-between gap-3 mb-4 pb-4 border-b border-base-300/50">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1.5">
                            <h3 class="font-bold text-base sm:text-lg truncate text-base-content">{{ order().code }}</h3>
                            <app-order-state-badge [state]="order().state" />
                        </div>
                        <p class="text-xs text-base-content/50">{{ formatDate(order().orderPlacedAt || order().createdAt) }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between gap-3 pt-4 border-t border-base-300/50">
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-lg sm:text-xl font-bold text-primary">{{ formatCurrency(order().totalWithTax) }}</p>
                    </div>
