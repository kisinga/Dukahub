...
export class StockAdjustmentsComponent implements OnInit {
  ...
  async handleLocationChange(locationId: string): Promise<void> {
    ...
    const currentStock = await this.stockAdjustmentService.getStockLevelForLocation(
      current.variantId,
      locationId,
    );
    ...
    const preserveNewStock =
      current.newStock !== undefined &&
  ...
  async handleUpdateLineItem(
    ...
    if (field === 'stockLocationId' && line.variantId && value) {
      ...
      const newNewStock =
        oldCurrentStock !== null
          ? oldNewStock - oldCurrentStock + (currentStock ?? 0)
          : oldNewStock;

      const newQuantityChange =
        currentStock !== null ? newNewStock - currentStock : line.quantityChange;
