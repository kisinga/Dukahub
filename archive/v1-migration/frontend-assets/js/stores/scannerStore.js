...
const scannerStoreLogic = {
  ...
  initScanner(modelInfo, videoEl) {
    ...
    this._errorMessage = '';
    this.status = 'idle';
    this.isConfigured = !!(modelInfo?.model && modelInfo?.weights && modelInfo?.metadata);
    if (!this.isConfigured) {
      this.updateStatus('error', 'Scanner disabled: Missing configuration URLs.');
      return;
    }
    ...
    if (!this.videoElement) {
      this.updateStatus('error', 'Scanner disabled: Video element reference missing.');
  ...
  async _fetchUrlAsFile(url, defaultFilename, type) {
    ...
    try {
      ...
      console.error(`Failed to fetch or create File for ${filename} (from ${url}):`, error);
      throw error;
    }
  },
  ...
  async _loadModel() {
    ...
    try {
      ...
      const [modelFile, weightsFile, metadataFile] = await Promise.all([
        this._fetchUrlAsFile(this.config.modelUrl, 'model.json', 'application/json'),
        // Pass 'weights.bin' as the default/fallback name
        this._fetchUrlAsFile(this.config.weightsUrl, 'weights.bin', 'application/octet-stream'),
        this._fetchUrlAsFile(this.config.metadataUrl, 'metadata.json', 'application/json'),
      ]);
      ...
      if (!loadedModel) throw new Error('tmImage.loadFromFiles() did not return a valid model.');
  ...
  async start() {
    ...
    try {
      ...
      if (!this.videoElement) throw new Error('Camera video element reference lost.');
      ...
      await new Promise((resolve, reject) => {
        ...
        this.videoElement.onerror = err => reject(new Error('Video element error during setup.'));
      });
  ...
  toggle() {
    if (this.isScanning) this.stop();
      ...
      if (this.status !== 'error' || this._errorMessage.includes('Configuration missing')) {
        this.start();
      } else {
        console.warn('Scanner toggle prevented due to persistent error state:', this._errorMessage);
      }
    }
  },

  async _detectLoop() {
    if (!this.model || !this.videoElement || !this.isScanning || !this.videoElement.srcObject)
      return;
    if (
    ...
    try {
      ...
      if (!rawModel) throw new Error('Could not get raw model object from proxy.');
      ...
      if (bestPrediction && bestPrediction.probability >= this.config.confidenceThreshold) {
        console.log(
  ...
  async _handleDetection(detectedLabel) {
    ...
    try {
      ...
      /* ... copy error handling ... */
      console.error(`Error fetching/verifying product "${detectedLabel}":`, error);
      const message =
        error.status === 404 ? `Item "${detectedLabel}" not found.` : `Error verifying item.`;
